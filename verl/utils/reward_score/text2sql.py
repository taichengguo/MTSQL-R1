...
# Removes trailing commas before } or ] (a common cause of JSON parse errors)
_TRAILING_COMMAS = re.compile(r",\s*([}\]])")

def extract_last_sql_code(solution_str: str) -> Optional[str]:
    """
    Return the *last* arguments.code string found inside <tool_call>...</tool_call>.
    Efficient: collects regex matches, then parses from the end backward and
    returns on the first valid 'code' found.
    """
    matches = list(_TOOL_CALL_BLOCK.finditer(solution_str))
    for m in reversed(matches):
        ...
        try:
            ...
            # Fallback: tolerant parse after removing trailing commas
            try:
                obj = json.loads(_TRAILING_COMMAS.sub(r"\1", block))
            except Exception:
                continue  # not valid JSON, try the previous match
...
def extract_code(text):
    ...
    try:
        ...
        # extract the code in the last function calling
        sql = extract_last_sql_code(text)
        if sql is not None:
            sql = sql.replace("\n", " ")
            sql = sql.strip()
            return sql
            ...
            try:
                ...
                print(f"extracting still error: {e}")
                return text
