...
SELECT LocalName FROM country	world_1
...
SELECT name from singer	concert_singer
SELECT song_release_year from singer where Name = "Joe Sharp" and Song_Name = "You"	concert_singer
SELECT count ( * ) FROM singer_in_concert AS T1 JOIN singer AS T2 ON T1.singer_id = T2.singer_id where T2.name = "Joe Sharp" GROUP BY T2.singer_id	concert_singer
SELECT country from singer where name = "Joe Sharp"	concert_singer

SELECT Continent FROM CONTINENTS	car_1
...
SELECT count ( distinct GovernmentForm ) from country	world_1
SELECT distinct GovernmentForm from country	world_1
SELECT count ( * ) from country where GovernmentForm = "Republic"	world_1
SELECT name from country where GovernmentForm = "Republic"	world_1
SELECT T2.language from country as T1 join countrylanguage as T2 on T1.Code = T2.CountryCode where T1.name = "Latvia"	world_1
...
SELECT T1.Airline, T2.DestAirport FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline GROUP BY T1.Airline HAVING count ( * ) < 200 limit 3	flight_2
...
SELECT name FROM Dogs	dog_kennels
SELECT name FROM Dogs WHERE breed_code = ( SELECT breed_code FROM Dogs GROUP BY breed_code ORDER BY count ( * ) ASC LIMIT 1 )	dog_kennels
SELECT name, gender FROM Dogs WHERE breed_code = ( SELECT breed_code FROM Dogs GROUP BY breed_code ORDER BY count ( * ) ASC LIMIT 1 )	dog_kennels
...
SELECT first_name, last_name FROM players	wta_1
...
SELECT count ( * ) FROM people AS T1 JOIN poker_player AS T2 ON T1.People_ID = T2.People_ID WHERE T1.Height > 200	poker_player
...
SELECT count ( * ) from country	world_1
SELECT countrycode from city where name = "Herat"	world_1
SELECT population from city where name = "Qandahar"	world_1

select id from tv_channel where country in ( SELECT country FROM tv_channel GROUP BY country HAVING count ( * ) > 2 )	tvshow
select package_option from tv_channel where country in ( SELECT country FROM tv_channel GROUP BY country HAVING count ( * ) > 2 )	tvshow
SELECT series_name FROM tv_channel order by series_name asc	tvshow
...
SELECT t2.name FROM hiring AS t1 JOIN shop AS t2 ON t1.shop_id = t2.shop_id GROUP BY t1.shop_id	employee_hire_evaluation
...
SELECT Year_of_Work FROM conductor where name = 'Robert Craft'	orchestra
...
SELECT * FROM CAR_MAKERS AS T1 JOIN MODEL_LIST AS T2 ON T1.Id = T2.Maker GROUP BY T1.Id HAVING count ( * ) > 3	car_1
SELECT T1.id FROM CAR_MAKERS AS T1 JOIN MODEL_LIST AS T2 ON T1.Id = T2.Maker GROUP BY T1.Id HAVING count ( * ) > 3	car_1
...
SELECT Num_of_Staff from museum	museum_visit
...
SELECT count ( StuID ) FROM student	pets_1
SELECT Age FROM student where LName = 'Smith'	pets_1
SELECT T2.petid ,age FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid WHERE T1.Lname = 'Smith'	pets_1

SELECT distinct T1.stuid FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid	pets_1
SELECT distinct T1.Fname, T1.Lname, T1.major, T1.age FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid	pets_1
SELECT Fname, Lname, major, age FROM student WHERE stuid NOT IN ( SELECT T1.stuid FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid JOIN pets AS T3 ON T3.petid = T2.petid WHERE T3.pettype = 'cat' )	pets_1
...
SELECT T2.Name, T1.course_id FROM course_arrange AS T1 JOIN teacher AS T2 ON T1.Teacher_ID = T2.Teacher_ID	course_teach
SELECT T2.Name , COUNT ( * ) FROM course_arrange AS T1 JOIN teacher AS T2 ON T1.Teacher_ID = T2.Teacher_ID GROUP BY T2.Name	course_teach
SELECT name from teacher where age < 30	course_teach

SELECT date FROM performance WHERE TYPE LIKE "%audition%"	orchestra
...
SELECT count ( * ) FROM Ref_template_types AS T1 JOIN Templates AS T2 ON T1.template_type_code = T2.template_type_code WHERE T1.template_type_description = "Presentation"	cre_Doc_Template_Mgt
...
SELECT T2.template_id, T2.Template_Type_Code FROM Ref_template_types AS T1 JOIN Templates AS T2 ON T1.template_type_code = T2.template_type_code WHERE T1.template_type_description = "Presentation"	cre_Doc_Template_Mgt
...
SELECT property_name, room_count FROM Properties WHERE property_type_code = "House" UNION SELECT property_name, room_count FROM Properties WHERE property_type_code = "Apartment" AND room_count > 1	real_estate_properties
SELECT avg ( agreed_selling_price ) FROM Properties WHERE property_type_code = "House" AND room_count > 1	real_estate_properties
...
SELECT first_name , last_name, state FROM Owners	dog_kennels
SELECT first_name , last_name, email_address FROM Owners	dog_kennels
...
SELECT COUNT ( DISTINCT Orchestra ) FROM orchestra	orchestra
SELECT COUNT ( Orchestra ) FROM orchestra WHERE Orchestra_ID NOT IN ( SELECT Orchestra_ID FROM performance )	orchestra
SELECT Orchestra_ID, Orchestra FROM orchestra WHERE Orchestra_ID NOT IN ( SELECT Orchestra_ID FROM performance )	orchestra
...
SELECT id FROM TV_Channel where id in ( SELECT channel FROM cartoon WHERE directed_by = 'Ben Jones' )	tvshow
...
SELECT owner_id,zip_code from Owners	dog_kennels
SELECT T1.owner_id , T2.dog_id ,T3.cost_of_treatment FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id JOIN Treatments AS T3 ON T2.dog_id = T3.dog_id	dog_kennels
SELECT T1.first_name,T1.owner_id FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id JOIN Treatments AS T3 ON T2.dog_id = T3.dog_id GROUP BY T1.owner_id ORDER BY sum ( T3.cost_of_treatment ) DESC LIMIT 1	dog_kennels
SELECT last_name from Owners where first_name = "Jaclyn"	dog_kennels
...
SELECT email_address FROM Professionals WHERE First_Name = "Taryn"	dog_kennels
SELECT state FROM Professionals WHERE First_Name = "Jayson"	dog_kennels
...
SELECT Name FROM country WHERE Continent = "Africa" ORDER BY LifeExpectancy LIMIT 1	world_1
SELECT GNP FROM country WHERE name = "Afghanistan"	world_1
...
SELECT RESULT FROM battle where name not in ( SELECT T1.name FROM battle AS T1 JOIN ship AS T2 ON T1.id = T2.lost_in_battle WHERE T2.location = 'English Channel' )	battle_death
SELECT bulgarian_commander FROM battle where name not in ( SELECT T1.name FROM battle AS T1 JOIN ship AS T2 ON T1.id = T2.lost_in_battle WHERE T2.location = 'English Channel' )	battle_death
SELECT date FROM battle where name not in ( SELECT T1.name FROM battle AS T1 JOIN ship AS T2 ON T1.id = T2.lost_in_battle WHERE T2.location = 'English Channel' )	battle_death
SELECT latin_commander FROM battle where name not in ( SELECT T1.name FROM battle AS T1 JOIN ship AS T2 ON T1.id = T2.lost_in_battle WHERE T2.location = 'English Channel' )	battle_death
...
SELECT T1.Fname FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid JOIN pets AS T3 ON T3.petid = T2.petid WHERE T3.pettype = 'dog'	pets_1
SELECT count ( T2.StuId ) FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid JOIN pets AS T3 ON T3.petid = T2.petid	pets_1
SELECT T3.pet_age FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid JOIN pets AS T3 ON T3.petid = T2.petid WHERE T1.Fname = "Linda"	pets_1
...
SELECT count ( * ) FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id WHERE T1.state = 'Virginia'	dog_kennels
SELECT T1.first_name, T1.last_name FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id WHERE T1.state = 'Virginia'	dog_kennels
SELECT T2.name FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id WHERE T1.state = 'Virginia'	dog_kennels
SELECT T2.breed_code FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id WHERE T2.name = "Bessie"	dog_kennels
SELECT T2.size_code FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id WHERE T2.name = "Bessie"	dog_kennels

SELECT count ( Record_Company ) FROM orchestra	orchestra
SELECT count ( Orchestra_ID ) FROM orchestra	orchestra
select orchestra from orchestra order by Year_of_Founded	orchestra
select year_of_founded from orchestra where orchestra = 'Royal Danish Orchestra'	orchestra
...
SELECT count ( * ) FROM Friend	network_1
SELECT T2.name FROM Friend AS T1 JOIN Highschooler AS T2 ON T1.student_id = T2.id	network_1
...
SELECT age FROM visitor where name = "Gonzalo Higuaín"	museum_visit
SELECT num_of_ticket FROM visitor AS t1 JOIN visit AS t2 ON t1.id = t2.visitor_id where visitor_id = 5	museum_visit
SELECT Total_spent FROM visitor AS t1 JOIN visit AS t2 ON t1.id = t2.visitor_id where name = "Arjen Robben"	museum_visit
...
SELECT * FROM Professionals WHERE state = 'Indiana'	dog_kennels
SELECT cell_number FROM Professionals WHERE first_name = "Taryn"	dog_kennels

SELECT count ( * ) FROM Templates	cre_Doc_Template_Mgt
SELECT Date_Effective_From, Date_Effective_To FROM Templates where Template_ID = 1	cre_Doc_Template_Mgt
SELECT Template_Type_Code FROM Templates where Template_ID = 4	cre_Doc_Template_Mgt
SELECT Version_Number FROM Templates where Template_ID = 0	cre_Doc_Template_Mgt
...
SELECT T2.Orchestra FROM conductor AS T1 JOIN orchestra AS T2 ON T1.Conductor_ID = T2.Conductor_ID	orchestra
SELECT Name FROM conductor	orchestra
SELECT Age FROM conductor WHERE Name = "Colin Davis"	orchestra
...
SELECT T3.area_code FROM contestants AS T1 JOIN votes AS T2 ON T1.contestant_number = T2.contestant_number JOIN area_code_state AS T3 ON T2.state = T3.state WHERE T1.contestant_name = 'Kelly Clauss'	voter_1
...
select department_id,department_name from Departments	student_transcripts_tracking
select T2.degree_summary_name from Departments as T1 JOIN Degree_Programs as T2 on T1.department_id = T2.department_id where T1.department_name = 'statistics'	student_transcripts_tracking
...
SELECT T3.Name , T2.Course, T1.teacher_ID FROM course_arrange AS T1 JOIN course AS T2 ON T1.Course_ID = T2.Course_ID JOIN teacher AS T3 ON T1.Teacher_ID = T3.Teacher_ID ORDER BY T3.Name	course_teach
...
SELECT Language FROM TV_Channel where series_name = 'Sky Music'	tvshow
SELECT distinct Package_Option FROM TV_Channel	tvshow
SELECT distinct Content FROM TV_Channel	tvshow

SELECT LANGUAGE FROM TV_Channel GROUP BY LANGUAGE ORDER BY count ( * ) ASC LIMIT 1	tvshow
SELECT LANGUAGE FROM TV_Channel GROUP BY LANGUAGE ORDER BY count ( * ) desc LIMIT 1	tvshow
SELECT content from tv_channel group by content order by count ( * ) desc limit 1	tvshow
SELECT content from tv_channel group by content order by count ( * ) limit 1	tvshow
SELECT series_name from tv_channel	tvshow
...
SELECT orchestra FROM orchestra order by orchestra	orchestra
SELECT Major_Record_Format FROM orchestra GROUP BY Major_Record_Format ORDER BY COUNT ( * ) ASC	orchestra

SELECT count ( Museum_ID ) FROM museum	museum_visit
SELECT Num_of_Staff FROM museum where museum.Museum_ID = 3	museum_visit
SELECT Open_year FROM museum where Name = 'Capital Plaza Museum'	museum_visit
...
SELECT Level_of_membership FROM visitor WHERE ID = 1	museum_visit
SELECT Name FROM visitor WHERE ID = 1	museum_visit
SELECT Age FROM visitor WHERE ID = 1	museum_visit
...
SELECT avg ( pet_age ) FROM pets WHERE PetType = 'cat'	pets_1
SELECT avg ( pet_age ) FROM pets WHERE PetType = 'dog'	pets_1
SELECT max ( pet_age ) FROM pets WHERE PetType = 'dog'	pets_1
SELECT max ( pet_age ) , pettype FROM pets GROUP BY pettype	pets_1
SELECT max ( pet_age ) FROM pets WHERE PetType = 'cat'	pets_1
...
select name from teacher order by Age asc limit 1	course_teach
select Hometown from teacher order by Age asc limit 1	course_teach
select Age from teacher where Name = 'Vicente Carretero'	course_teach

SELECT fname, lname FROM student WHERE sex = 'F'	pets_1
SELECT DISTINCT city_code FROM student	pets_1
SELECT T3.weight FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid JOIN pets AS T3 ON T2.petid = T3.petid WHERE T1.sex = 'F'	pets_1
...
SELECT FlightNo FROM flights where SourceAirport = "CVO" and DestAirport = "SNA"	flight_2
SELECT * FROM flights where SourceAirport = "CVO" and DestAirport = "DFW"	flight_2
...
SELECT T2.age, t2.hometown FROM course_arrange AS T1 JOIN teacher AS T2 ON T1.Teacher_ID = T2.Teacher_ID GROUP BY T2.Name HAVING COUNT ( * ) > = 2	course_teach
SELECT t1.grade FROM course_arrange AS T1 JOIN teacher AS T2 ON T1.Teacher_ID = T2.Teacher_ID order by t2.age desc limit 1	course_teach

SELECT max ( num_of_ticket ) FROM visit	museum_visit
SELECT avg ( num_of_ticket ) FROM visit	museum_visit

SELECT count ( * ) from ship	battle_death
SELECT name from ship	battle_death
...
SELECT Name FROM country WHERE continent = "Europe"	world_1
...
SELECT age from singer where name = "Joe Sharp"	concert_singer
select count ( * ) from singer where country = "United States"	concert_singer
select count ( * ) from singer where Is_male = "True"	concert_singer
...
select feature_description from Other_Available_Features where feature_name = "AirCon"	real_estate_properties
select count ( * ) from ( select distinct feature_id from Other_Available_Features )	real_estate_properties
select feature_type_code from Other_Available_Features where feature_name = "AirCon"	real_estate_properties
...
SELECT count ( * ) FROM Dogs WHERE abandoned_yn = 1 AND dog_id NOT IN ( SELECT dog_id FROM Treatments )	dog_kennels
SELECT T1.dog_id, T2.treatment_type_code FROM Dogs AS T1 JOIN Treatments AS T2 ON T1.dog_id = T2.dog_id WHERE abandoned_yn = 1	dog_kennels
...
SELECT sum ( cost_of_treatment ) from treatments	dog_kennels
select T1.first_name, T1.last_name from Professionals as T1 join Treatments as T2 on T1.professional_id = T2.professional_id order by T2.cost_of_treatment asc limit 1	dog_kennels
select dog_id from treatments order by cost_of_treatment asc limit 1	dog_kennels

SELECT ContId , Continent FROM CONTINENTS	car_1
SELECT T1.Continent , count ( * ) FROM CONTINENTS AS T1 JOIN COUNTRIES AS T2 ON T1.ContId = T2.Continent GROUP BY T1.ContId	car_1
SELECT T1.Continent , count ( * ) FROM CONTINENTS AS T1 JOIN COUNTRIES AS T2 ON T1.ContId = T2.Continent GROUP BY T1.ContId ORDER BY count ( * ) DESC LIMIT 1	car_1
...
SELECT T1.paragraph_id FROM Paragraphs AS T1 JOIN Documents AS T2 ON T1.document_id = T2.document_id WHERE T2.Document_Name = 'Welcome to NY'	cre_Doc_Template_Mgt
...
SELECT count ( distinct Language ) from countrylanguage	world_1
SELECT max ( Percentage ) from countrylanguage	world_1
...
SELECT email_address from owners where owner_id = 1	dog_kennels
SELECT weight from dogs where dog_id = 1	dog_kennels
SELECT age from dogs where dog_id = 1	dog_kennels
...
SELECT state FROM Professionals	dog_kennels
...
SELECT count ( Course_ID ) FROM course	course_teach
SELECT Course FROM course	course_teach
...
SELECT * FROM Documents AS T1 JOIN Templates AS T2 ON T1.Template_ID = T2.Template_ID	cre_Doc_Template_Mgt
