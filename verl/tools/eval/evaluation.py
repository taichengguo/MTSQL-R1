...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    with open(predict) as f:
        plist = []
        pseq_one = []
        # enforce the size of pred to be the same as gt
        gt_index = 0
        leng_session_in_gt = len(glist[gt_index])

        for l in f.readlines():
            if len(l.strip()) == 0 and leng_session_in_gt <= 0:
    ...
    # TODO print difficulty
    # hard_array = []
    # for dialogue in glist:
    #     for g in dialogue:
    #         g_str, db = g
    #         db_name = db
    ...
    #         hard_array.append(hardness)
    # # save arrary to pickle
    # import pickle
    # with open('hard_array.pkl', 'wb') as f:
    #     pickle.dump(hard_array, f)
    ...
    # write score of each sample to a file
    with open(f"exact.txt", "a") as f:
        f.write(json.dumps(each_exact) + "\n")
    with open(f"exec.txt", "a") as f:
        f.write(json.dumps(each_exec) + "\n")
...
def eval_exact_match_score(db, p_str, g_str, dataset="cosql"):
    # TODO: set to your local path to the text2sql dataset directory
    dataset_dir = f"PATH/TO/database/{dataset}/"
    ...
    kmaps = build_foreign_key_map_from_json(os.path.join(dataset_dir, 'tables.json'))
    kmap = kmaps[db_name]
    g_valid_col_units = build_valid_col_units(g_sql['from']['table_units'], schema)
    g_sql = rebuild_sql_val(g_sql)
    g_sql = rebuild_sql_col(g_valid_col_units, g_sql, kmap)
